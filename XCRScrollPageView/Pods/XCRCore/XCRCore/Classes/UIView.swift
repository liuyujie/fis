...
extension UIView {
    ...
    public func centerXInSuperView() {
        guard let parentView = superview else {
            assertionFailure("XCRKit Error: The view \(self) doesn't have a superview")
            return
        }
...
extension UIView {
    ...
    /// XCRKit
    public func cornerRadius(radius: CGFloat, borderWidth: CGFloat = 0.0) {
...
extension UIView {
    /// XCRKit [UIRectCorner.TopLeft, UIRectCorner.TopRight]
    ...
    /// XCRKit - Mask square/rectangle UIView with a circular/capsule cover, with a border of desired color and width around it
    public func roundView(withBorderColor color: UIColor? = nil, withBorderWidth width: CGFloat? = nil) {
        self.cornerRadius(radius: min(self.frame.size.height, self.frame.size.width) / 2)
        self.layer.borderWidth = width ?? 0
        self.layer.borderColor = color?.cgColor ?? UIColor.clear.cgColor
    }

    /// XCRKit - Remove all masking around UIView
